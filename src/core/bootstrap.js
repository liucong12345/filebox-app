...
export default function Initializer () {
  ...
  store.dispatch('setLang', storage.get(APP_LANGUAGE, ['zh-cn', 'zh'].includes((navigator.language || navigator.browserLanguage).toLowerCase()) ? 'zh-CN' : (['en-us', 'en'].includes((navigator.language || navigator.browserLanguage).toLowerCase()) ? 'en-US' : 'en-US')))
  // last step
}
